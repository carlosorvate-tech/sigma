...
// 3. RECUPERAR E AJUSTAR INDEX.HTML
let html = fs.readFileSync('index.html', 'utf8');

// Ajustar o banner para chamar openOficinasModal
html = html.replace("document.getElementById('vehicleOficinaText').innerText = ultimaOficina;", 
  "const oficinas = state.oficinas || [];" +
  "const ofBase = oficinas.find(function(o) { return o.isBase || o.Flag_Oficina_Base; });" +
...
// Adicionar botão no Menu Lateral
if (!html.includes('openOficinasModal()')) {
  const btnMenu = '<button onclick="openOficinasModal()" class="w-full text-left px-3 py-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 text-xs flex items-center gap-2 transition"><i data-lucide="building-2" class="w-4 h-4 text-sky-400"></i><span>Oficinas Credenciadas</span></button>\n          <button onclick="openVehicleModal()"';
  html = html.replace('<button onclick="openVehicleModal()"', btnMenu);
}

// Injetar o Modal e Script antes de </body>
...
console.log('✅ Arquivos index.html e Code.gs compilados com sucesso!');
