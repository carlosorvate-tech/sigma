...
// 1. RESTAURAR DA VERSÃO ESTÁVEL V101 VALIDADA
if (fs.existsSync('backups/checkpoint_v101_infinitus_production_validated/index.html')) {
  fs.copyFileSync('backups/checkpoint_v101_infinitus_production_validated/index.html', 'index.html');
  fs.copyFileSync('backups/checkpoint_v101_infinitus_production_validated/Code.gs', 'Code.gs');
} else if (fs.existsSync('backups/checkpoint_v101_pre_prestadores_module/index.html')) {
  fs.copyFileSync('backups/checkpoint_v101_pre_prestadores_module/index.html', 'index.html');
  fs.copyFileSync('backups/checkpoint_v101_pre_prestadores_module/Code.gs', 'Code.gs');
}

// 2. ATUALIZAR CODE.GS COM AS FUNÇÕES DE OFICINAS
...
// 3. RECUPERAR E ESTRUTURAR INDEX.HTML COM SINTAXE BLINDADA
let html = fs.readFileSync('index.html', 'utf8');

// Atualizar Banner de forma segura
if (html.includes("document.getElementById('vehicleOficinaText').innerText = ultimaOficina;")) {
  html = html.replace(
    "document.getElementById('vehicleOficinaText').innerText = ultimaOficina;",
...
if (html.includes('<button onclick="openVehicleModal()"') && !html.includes('openOficinasModal()')) {
  html = html.replace('<button onclick="openVehicleModal()"', menuOficinaBtn);
}

// Injetar Funções JS e Modal antes do </body>
const jsEModalOficinas = `
  ...
  <script>
    ...
    function editarOficina(idOficina) {
      ...
      document.getElementById('ofiTipoAtendimento').value = o.tipoAtendimento || o.Tipo_Atendimento || 'Mecânica Geral & Injeção';
...
if (!html.includes('id="modalOficinasSIGMA"')) {
  html = html.replace('</body>', jsEModalOficinas + '\n</body>');
}

fs.writeFileSync('index.html', html, 'utf8');
fs.writeFileSync('App.html', html, 'utf8');
console.log('✅ Restauração e implementação concluídas com sucesso!');
