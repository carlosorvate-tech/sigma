  ...
  try {
    ...
    const htmlTemplate = `
      ...
      <body>
        ...
        <div class="section-title">4. Panorama Geral do Veículo (Auditoria Marco Zero)</div>
        <table class="saude-table">
          ...
          <tr>
            <td>${saude.motor}</td>
            <td>${saude.transmissao}</td>
            <td>${saude.freios}</td>
            <td>${saude.suspensao}</td>
            <td>${saude.arrefecimento}</td>
          </tr>
        </table>
        <div style="font-size: 8pt; color: #64748b; text-align: center; margin-top: -15px; margin-bottom: 20px;">
          * Status operacional auditado pelo aplicativo SIGMA. Avaliar subsistemas fora de conformidade.
        </div>
